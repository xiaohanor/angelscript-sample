struct FPinballPredictionDispatchedLaunch
{
...
/**
 * We can't dispatch impacts from the resolver in prediction, because it would call
 * events multiple times per frame, and at the wrong visual time.
 * Instead, this capability runs after movement, and dispatches the impact events when the visual ball
 * has gone past the time when the impact occurred.
 */
class UPinballPredictionImpactsCapability : UPinballMagnetDronePredictionCapability
{
	...
	TArray<FPinballPredictionDispatchedLaunch> DispatchedLaunches;
	...
	void TickActive(float DeltaTime)
	{
		...
		if(Proxy.InitialLaunchData.IsValid())
		{
			if(CanDispatchLaunch(Proxy.InitialLaunchData))
				DispatchLaunch(Proxy.InitialLaunchData);
		}
	...
	{
		for(const FPinballPredictionDispatchedLaunch& DispatchedLaunch : DispatchedLaunches)
		{
	...
	void DispatchLaunch(FPinballBallLaunchData LaunchData)
	{
		...
		FPinballPredictionDispatchedLaunch DispatchedLaunch;
