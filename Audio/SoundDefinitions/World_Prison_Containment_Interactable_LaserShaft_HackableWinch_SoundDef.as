...
UCLASS(Abstract)
class UWorld_Prison_Containment_Interactable_LaserShaft_HackableWinch_SoundDef : USoundDefBase
{
	...
	private void QueryMovement(float DeltaSeconds)
	{
		...
		if(WinchInput.Z != 0)
		{
			if(!MovingInstanceIsPlaying(VerticalMovingEventInstance))
			{
		...
		{
			if(!MovingInstanceIsPlaying(HorizontalMovingEventInstance))
			{
				OnStartMovingHorizontal();			
			}
		}
		else if(MovingInstanceIsPlaying(HorizontalMovingEventInstance))
		{			
	...
	{
		Speed = VerticalSpeed;
		//Direction = GetVerticalMovementDirection();
	}	
