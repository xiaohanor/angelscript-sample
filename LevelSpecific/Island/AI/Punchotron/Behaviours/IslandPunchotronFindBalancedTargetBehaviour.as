...
class UIslandPunchotronFindBalancedTargetBehaviour : UBasicBehaviour
{
	...
	{
		TListedActors<AAIIslandPunchotronSidescroller> Punchotrons;
		if (Punchotrons.Num() == 1 && TargetComp.HasValidTarget())
			return;
		
		// Don't switch target in the middle of performing an attack
	...
	AHazeActor GetBestTarget()
	{
		...
		{
			if (!TargetComp.IsValidTarget(KnownTarget))
				continue;
