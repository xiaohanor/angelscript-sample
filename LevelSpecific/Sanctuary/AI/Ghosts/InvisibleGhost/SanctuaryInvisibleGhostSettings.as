class USanctuaryInvisibleGhostSettings : UHazeComposableSettings
{
	UPROPERTY(Category = "Attack|Charge")
	float ChargeRange = 1000.0;

	UPROPERTY(Category = "Attack|Charge")
	float ChargeMinRange = 500.0;
	...
	// How long indication is done
	UPROPERTY(Category = "Invisibility")
	float IndicateDuration = 0.6;

	// How long to wait before doing other actions after indicating
	UPROPERTY(Category = "Invisibility")
	float IndicateWait = 1.0;

	// The interval at which indicators are shown while invisible
	UPROPERTY(Category = "Invisibility")
	float IndicateInterval = 2;
};
