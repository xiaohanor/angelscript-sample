class UPinballMagnetAttachedMovementResolver : USweepingMovementResolver
{
	default RequiredDataType = UPinballMagnetAttachedMovementData;
	private const UPinballMagnetAttachedMovementData MagnetMoveData;

	void PrepareResolver(const UBaseMovementData Movement) override
	{
		Super::PrepareResolver(Movement);
		MagnetMoveData = Cast<UPinballMagnetAttachedMovementData>(Movement);
	}
...
#if !RELEASE
			if(!MagnetMoveData.IsRerun() && DevToggleMagnetDrone::DrawValidatedUp.IsEnabled())
			{
	...
	private bool TryGetConstrainToWithinZoneAtPoint(FVector Point, UDroneMagneticSurfaceComponent MagneticSurfaceComponent, UDroneMagneticZoneComponent& OutClosestZone) const
	{
		...
		for(auto Zone : MagneticSurfaceComponent.MagneticZones)
		{
			...
			if(DistanceToTarget < ClosestZoneDistance)
			{
				...
				if(DistanceToCurrentLocation > MagnetMoveData.ShapeSizeForMovement)
