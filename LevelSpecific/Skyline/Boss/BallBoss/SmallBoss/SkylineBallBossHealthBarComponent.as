class USkylineBallBossHealthBarComponent : UActorComponent
{
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "GUI")
	private TSubclassOf<UBossHealthBarDefault> HealthBarWidgetClass;
	...
	UBossHealthBarDefault BallBossHealthBar;
	...
	void SetNewName(FText NewName)
	{
		if (BallBossHealthBar != nullptr)
			BallBossHealthBar.BossNameLabel.SetText(NewName);
	}
	...
	void OnTakeDamage(AHazeActor Actor, AHazeActor Instigator, float Damage, EDamageType DamageType)
	{
		...
		if (HealthComp.IsAlive())
		{
			...
			UpdateHealthBar(BallBossHealthBar);
		}
		else // Dead
		{
			RemoveHealthBars();
	...
	void UpdateHealthBarVisibility()
	{
		...
		if (IsBossHealthBar())
		{
			...
			if (BallBossHealthBar == nullptr)
			{
				BallBossHealthBar = Widget::AddFullscreenWidget(HealthBarWidgetClass);
				BallBossHealthBar.InitBossHealthBar(BossHealthBarDesc, HealthComp.CurrentHealth);
			}
		...
		else
		{
			...
			if (BallBossHealthBar != nullptr)
			{
				Widget::RemoveFullscreenWidget(BallBossHealthBar);
				BallBossHealthBar = nullptr;	
			}
		}
	}
	...
	void UpdateHealthBarSettings()
	{
		...
		if (BallBossHealthBar != nullptr)
			BallBossHealthBar.SetHealthAsDamage(HealthComp.CurrentHealth);
	}
