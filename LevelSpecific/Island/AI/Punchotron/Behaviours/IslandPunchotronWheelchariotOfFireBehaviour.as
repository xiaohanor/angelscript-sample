class UIslandPunchotronWheelchariotOfFireAttackBehaviour : UBasicBehaviour
{
	...
	void TickActive(float DeltaTime)
	{
		...
		if (ActiveDuration < Settings.WheelchairKickAttackTelegraphDuration)
		{
			...
			{
				//UIslandPunchotronEffectHandler::Trigger_OnSpinningAttackTelegraphingStart(Owner, FIslandPunchotronSpinningAttackTelegraphingParams(Punchotron.EyeTelegraphingLocation, TargetComp.Target));  // TODO: fix event name or create new
		...
		// Action phase - activate hitbox during time window
			...
			ActiveDuration < Settings.WheelchairKickAttackTelegraphDuration + 11.0)
		{
			if (!Owner.IsCapabilityTagBlocked(n"DamagePlayerOnTouch"))
		...
		if (Owner.bHazeEditorOnlyDebugBool)
		{
			PrintScaled(f"WheelchariotOfFire, ActiveDuration={ActiveDuration:.2} out of " + Settings.WheelchairKickAttackDuration, 0.0, Scale = 2.f);
	...
	{	
		...
		if (ActiveDuration < Settings.WheelchairKickAttackTelegraphDuration + 11.0)
		{
			DestinationComp.MoveAlongSpline(SplineFollowComp.Spline, 3500.0);
	...
	private void UpdateTargetLocation(const float DeltaTime)
	{
		...
		// initial target location TODO: move to just when telegraph ends
		if (CurrentTargetLocation.IsZero())
		{
			CurrentTargetLocation = TargetComp.Target.ActorLocation + (TargetComp.Target.ActorLocation - Owner.ActorLocation).GetSafeNormal2D() * Settings.WheelchairKickAttackTargetOffset; // this may be in the ground or up in the air.
		}
		else
		{
			FVector DesiredTargetLocation = TargetComp.Target.ActorLocation + (TargetComp.Target.ActorLocation - Owner.ActorLocation).GetSafeNormal() * Settings.WheelchairKickAttackTargetOffset; // this may be in the ground or up in the air.
			float LerpFactor = 25.0;
			FVector ToCurrentTargetDir = (CurrentTargetLocation - Owner.ActorLocation).GetSafeNormal();
			FVector ToDesiredTargetDir = (DesiredTargetLocation - Owner.ActorLocation).GetSafeNormal();
			FVector NewDir = ToCurrentTargetDir.RotateTowards(ToDesiredTargetDir, DeltaTime * LerpFactor);
			CurrentTargetLocation = Owner.ActorLocation + NewDir * Settings.WheelchairKickAttackTargetOffset;
			if (!PathingSettings.bIgnorePathfinding)
			{
				FVector NavmeshLocation;
			...
			//Debug::DrawDebugSphere(DesiredTargetLocation, Settings.WheelchairKickAttackHitRadius, LineColor = FLinearColor::Blue, Duration = 0.0);
		}

		//Debug::DrawDebugSphere(CurrentTargetLocation, Settings.WheelchairKickAttackHitRadius, LineColor = FLinearColor::Black, Duration = 0.0);
	}
