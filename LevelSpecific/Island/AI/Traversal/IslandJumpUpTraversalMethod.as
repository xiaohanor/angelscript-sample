class UIslandJumpUpTraversalMethod : UTraversalMethod
{	
	...
	default MaxRange = 750.0;
	float MaxLeapSpeed = 2*1500.0;
	float Gravity = 2*982.0;
	float IdealAngle = 40.0;
	float DefaultHeight = 40.0;

	UTraversalManager TraversalManager;
	FDirectTraversalTrajectoryOptions DirectTrajectories;

	default MethodName = n"IslandJumpUpTraversalMethod";
	...
	void AddTraversalPath(UScenepointComponent From, UScenepointComponent To) override
	{
		...
		Print("Added jump up destination");
	}

	private bool GetTrajectory(FVector From, FVector To, float Height, float GravityFactor, FTraversalTrajectory& OutTrajectory)
	{
		...
		FVector Vel = Trajectory::CalculateVelocityForPathWithHeight(From, To, Gravity * GravityFactor, VDist + Height);
		...
		OutTrajectory.LaunchVelocity = Vel; //LeapDir * LeapSpeed;
