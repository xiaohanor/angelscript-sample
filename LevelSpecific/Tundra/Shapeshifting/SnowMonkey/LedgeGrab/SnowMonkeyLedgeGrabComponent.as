...
class USnowMonkeyLedgeGrabComponent : UActorComponent
{
	USnowMonkeyLedgeGrabSettings Settings;
	USnowMonkeyLedgeGrabClimbSettings ClimbSettings;
	USnowMonkeyLedgeGrabEnterDropSettings DropSettings;
	...
	*/
	protected ESnowMonkeyLedgeGrabState CurrentState;
	protected ESnowMonkeyLedgeGrabClimbState CurrentClimbState;

	FSnowMonkeyLedgeGrabData Data;
	FSnowMonkeyLedgeGrabClimbData ClimbData;

	UPROPERTY(BlueprintReadOnly)
	FSnowMonkeyLedgeGrabAnimationData AnimData;

	bool bLedgeGrabDashCompleted = false;

	const float DebugDuration = 2.0;
	float DashDirectionSign = 0;

	UFUNCTION(BlueprintOverride)
	void BeginPlay()
	{
		Settings = USnowMonkeyLedgeGrabSettings::GetSettings(Cast<AHazeActor>(Owner));
		ClimbSettings = USnowMonkeyLedgeGrabClimbSettings::GetSettings(Cast<AHazeActor>(Owner));
		DropSettings = USnowMonkeyLedgeGrabEnterDropSettings::GetSettings(Cast<AHazeActor>(Owner));
	...
	ESnowMonkeyLedgeGrabState GetState() const property
	{
		return CurrentState;
	}

	ESnowMonkeyLedgeGrabClimbState GetClimbState() const property
	{
		return CurrentClimbState;
	}

	void SetState(ESnowMonkeyLedgeGrabState NewState) property
	{
		CurrentState = NewState;
		AnimData.State = CurrentState;
	}

	void SetClimbUpState(ESnowMonkeyLedgeGrabClimbState NewState) property
	{
	...
	void ResetLedgeGrab()
	{
		CurrentState = ESnowMonkeyLedgeGrabState::None;
		CurrentClimbState = ESnowMonkeyLedgeGrabClimbState::Inactive;
		Data.Reset();
		ClimbData.Reset();
		AnimData.Reset();
	}
	...
	bool TraceForLedgeGrab(AHazePlayerCharacter Player, FVector Direction, FSnowMonkeyLedgeGrabData& LedgeGrabData, bool bDebug = false, float OverrideTraceDistance = 0.0)
	{
		return TraceForLedgeGrabAtLocation(Player, Direction, Player.ActorLocation, LedgeGrabData, bDebug, OverrideTraceDistance);
	}

	//Trace for LedgeGrab from defined location
	bool TraceForLedgeGrabAtLocation(AHazePlayerCharacter Player, FVector Direction, FVector Location, FSnowMonkeyLedgeGrabData& LedgeGrabData, bool bDebug = false, float OverrideTraceDistance = 0.0)
	{
		if (Player == nullptr)
			return false;
				...
				*/
				if (!TopTraceForwardHit.bBlockingHit)
				{
					...
					if (TopTraceDownwardHit.bBlockingHit)
					{
						...
						if (bDebug)
						{
							// Debug::DrawDebugLine(LedgeGrabData.LedgeLocation, LedgeGrabData.LedgeLocation + Forward * 500.0, FLinearColor::LucBlue);
							// Debug::DrawDebugCoordinateSystem(LedgeGrabData.LedgeLocation, Rotation, 50.0, 2.0);
							// Debug::DrawDebugCoordinateSystem(LedgeGrabData.LedgeLocation, RealRotation, 50.0, 2.0);
						}	
	...
	bool TraceClimbUp(AHazePlayerCharacter Player, FSnowMonkeyLedgeGrabClimbData& InClimbData, bool bDebug = false)
	{
...
struct FSnowMonkeyLedgeGrabData
{
...
struct FSnowMonkeyLedgeGrabClimbData
{
...
struct FSnowMonkeyLedgeGrabAnimationData
{
	// Movement state of ledge grab
	UPROPERTY()
	ESnowMonkeyLedgeGrabState State;

	//What kind of climb up we are performing
	UPROPERTY()
	ESnowMonkeyLedgeGrabClimbState ClimbState;
	...
	void Reset()
	{
		State = ESnowMonkeyLedgeGrabState::None;
		ClimbState = ESnowMonkeyLedgeGrabClimbState::Inactive;
		ShimmyScale = 0.0;
	}
}

enum ESnowMonkeyLedgeGrabState
{
	None,
	EnterDrop,
	LedgeGrab,
...
enum ESnowMonkeyLedgeGrabClimbState
{
	Inactive,
	ClimbToIdle,
	ClimbToIdleCrouch,
