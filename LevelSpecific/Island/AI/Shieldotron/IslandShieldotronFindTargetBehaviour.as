...
#if !RELEASE
enum EIslandShieldotronTargetSelectedBy
{
	AggroTarget, // AggroTarget is managed by TargetComp
	Colour,
	OnlyPotentialTarget,
	RebalancedTarget,
	LastAttacker,
...
#endif
class UIslandShieldotronFindTargetBehaviour : UBasicBehaviour
{
	...
	void TickActive(float DeltaTime)
	{
		...
		// Target player with opposite colour preferably
		if (Target == nullptr && !ForceFieldComp.IsDepleted() && ForceFieldComp.CurrentType != EIslandForceFieldType::Both)
		{
			for (AHazeActor PotentialTarget : PotentialTargets)
			{
				AHazePlayerCharacter Player = Cast<AHazePlayerCharacter>(PotentialTarget);
				if (IslandForceField::GetPlayerForceFieldType(Player) == ForceFieldComp.CurrentType)
					continue;
				
				Target = Player;
#if !RELEASE		
				TargetSelectedBy = EIslandShieldotronTargetSelectedBy::Colour;
#endif
				break;
			}
		}			
