class UIslandRedBlueOverheatAssaultShootBulletCapability : UHazePlayerCapability
{
	...
	{
		OverheatUserComponent.OverheatAlpha += OverheatAssaultSettings.OverheatAmountPerShot;
		if(OverheatUserComponent.OverheatAlpha >= 1.0)
		{
			OverheatUserComponent.OverheatAlpha = 1.0;
			OverheatUserComponent.SetIsOverheated(true);
		}
		...
		BulletParams.BulletInitialSpeed = OverheatAssaultSettings.BulletInitialSpeed;
		BulletParams.BulletSpeedAcceleration = OverheatAssaultSettings.BulletSpeedAcceleration;
		BulletParams.BulletSpeedMax = OverheatAssaultSettings.BulletSpeedMax;
		BulletParams.BulletDamageMultiplier = OverheatAssaultSettings.BulletDamageMultiplier;
	...
	float GetCurrentCooldownBetweenBullets(float TimeSinceShouldHaveShot) const
	{
		...
		const float CurveAlpha = Math::Clamp(TimeSinceStartShooting / OverheatAssaultSettings.CoolDownBetweenBulletsCurveDuration, 0.0, 1.0);
		const float CurveValue = OverheatAssaultSettings.CoolDownBetweenBulletsCurve.GetFloatValue(CurveAlpha);
		return Math::Lerp(OverheatAssaultSettings.StartCoolDownBetweenBullets, OverheatAssaultSettings.TargetCoolDownBetweenBullets, CurveValue);
	}
}