class ASummitAcidActivatorOuroborosSingle : ASummitAcidActivatorActor
{
	...
	default DisableComp.AutoDisableRange = 80000.0;
	...
	void OnAcidActivatorStarted(AAcidActivator Activator) override
	{
		...
		if(HasControl())
			SetRotationValues(TargetRotation + 180 * RotationDirection);
	}

	private void SetRotationValues(float NewTargetRotation)
	{
		// Commented this out John, it was never set to true so it just broke it? // Frankles
		// if (!bCanRotate)
		// 	return;
	...
	void SetCanRotate(bool bNewCanRotate)
	{
		bCanRotate = bNewCanRotate;
	}
	...
	UFUNCTION()
	void SetPermanentInactive()
	{
		bPermanentInactive = true;
	}
};