class UIslandRedBlueAutoShotgunSettings : UHazeComposableSettings
{
	UPROPERTY(Category = "Shooting")
	float RumbleAmount = 0.5;

	UPROPERTY(Category = "Shooting")
	int AmountOfShotgunShots = 12;

	UPROPERTY(Category = "Shooting")
	float MaxRandomSpreadInDegrees = 6.0;

	UPROPERTY(Category = "Bullet")
	float BulletInitialSpeed = 15000;
	...
	UPROPERTY(Category = "Bullet")
	float BulletSpeedMax = 30000;

	// When spawning the bullet, if the current bullet's forward is above this angle degrees from the muzzle forward, it will get clamped
	UPROPERTY(Category = "Bullet")
	float BulletMaxDegreeOffsetFromMuzzle = 30.0;

	UPROPERTY(Category = "Bullet")
	float StartCoolDownBetweenBullets = 0.2;

	// How long between each projectile until we can shoot again
	UPROPERTY(Category = "Bullet")
	float TargetCoolDownBetweenBullets = 0.2;
