class USkylineCrowdSurfingVolumeVisualizer : UHazeScriptComponentVisualizer
{
	default VisualizedClass = USkylineCrowdSurfingVolumeVisualizerComponent;
	...
	{
		auto CrowdSurfingVolume = Cast<ASkylineCrowdSurfingVolume>(InComponent.Owner);

		DrawDashedLine(CrowdSurfingVolume.ActorLocation, CrowdSurfingVolume.ActorTransform.TransformPositionNoScale(CrowdSurfingVolume.TargetLocation), FLinearColor::LucBlue, 10.0, 5.0, false, 40);

		SetHitProxy(n"TargetProxy", EVisualizerCursor::CardinalCross);
		DrawWireDiamond(CrowdSurfingVolume.ActorTransform.TransformPositionNoScale(CrowdSurfingVolume.TargetLocation), CrowdSurfingVolume.ActorRotation, 100.0, FLinearColor::Blue, 5.0);
		ClearHitProxy();
	}
	...
	{
		auto CrowdSurfingVolume = Cast<ASkylineCrowdSurfingVolume>(EditingComponent.Owner);

		if(bIsHandleSelected)
		{
			OutLocation = CrowdSurfingVolume.ActorTransform.TransformPositionNoScale(CrowdSurfingVolume.TargetLocation);
			return true;
		}
	...
	bool HandleInputDelta(FVector& DeltaTranslate, FRotator& DeltaRotate, FVector& DeltaScale)
	{
		...
		if(bIsHandleSelected)
		{
			CrowdSurfingVolume.TargetLocation += CrowdSurfingVolume.ActorTransform.InverseTransformVectorNoScale(DeltaTranslate);
			return true;
		}
...
class USkylineCrowdSurfingVolumeVisualizerComponent : UHazeEditorRenderedComponent
{
	UFUNCTION(BlueprintOverride)
	void CreateEditorRenderState()
	{
#if EDITOR
		auto CrowdSurfingVolume = Cast<ASkylineCrowdSurfingVolume>(Owner);
		if (CrowdSurfingVolume == nullptr)
			return;

		float Thickness = 5.0;
		FLinearColor Color = FLinearColor::Blue;

		FTransform Transform = CrowdSurfingVolume.ActorTransform;
		FCollisionShape Shape = CrowdSurfingVolume.Shape.CollisionShape;

		SetActorHitProxy();
		switch (CrowdSurfingVolume.Shape.Type)
		{
...
UCLASS(Abstract)
class ASkylineCrowdSurfingVolume : AHazeActor
{
	...
	UPROPERTY(DefaultComponent)
	USkylineCrowdSurfingVolumeVisualizerComponent VisualizerComp;

	UPROPERTY(DefaultComponent)
	UHazeRequestCapabilityOnPlayerComponent RequestCapabilityComp;
//	default RequestCapabilityComp.PlayerCapabilities.Add(n"SkylineCrowdSurfingCapability");
	...
	FVector GetPushTargetLocation() property
	{
