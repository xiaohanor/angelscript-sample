...
class UPlayerLedgeGrabClimbIntoMovementCapability : UHazePlayerCapability
{
	/*
	...
	UFUNCTION(BlueprintOverride)
	bool ShouldActivate(FPlayerLedgeGrabClimbActivationParams& ActivationParams) const
	{
		...
		if(!VerifyInputSizeAndDirection() && !LedgeGrabComp.bSnowMonkeyLedgeGrabActivated)
			return false;

		if(!IsActioning(ActionNames::MovementJump) && !LedgeGrabComp.bSnowMonkeyLedgeGrabActivated)
			return false;
	...
	bool ShouldDeactivate() const
	{
		...
		if (LedgeGrabComp.State != EPlayerLedgeGrabState::Climb)
			return true;
	...
	UFUNCTION(BlueprintOverride)
	void OnActivated(FPlayerLedgeGrabClimbActivationParams ActivationParams)
	{
		Player.BlockCapabilities(BlockedWhileIn::LedgeGrab, this);
		Player.ConsumeButtonInputsRelatedTo(ActionNames::MovementJump);

		LedgeGrabComp.State = EPlayerLedgeGrabState::Climb;
		LedgeGrabComp.ClimbUpState = ActivationParams.ClimbData.bClimbIntoCrouch ? EPlayerLedgeGrabClimbState::ClimbToMovingCrouch : EPlayerLedgeGrabClimbState::ClimbToMoving;
		...
		if(LedgeGrabComp.GetClimbState() == EPlayerLedgeGrabClimbState::ClimbToMovingCrouch)
