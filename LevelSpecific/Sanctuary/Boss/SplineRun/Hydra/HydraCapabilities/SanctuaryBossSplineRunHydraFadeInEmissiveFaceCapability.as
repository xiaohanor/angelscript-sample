class USanctuaryBossSplineRunHydraFadeInEmissiveFaceCapability : UHazeCapability
{
	default CapabilityTags.Add(CapabilityTags::GameplayAction);

	ASanctuaryBossSplineRunHydra HydraHead;
	...
	bool bSentStartEvent = false;

	UFUNCTION(BlueprintOverride)
	void Setup()
	{
		HydraHead = Cast<ASanctuaryBossSplineRunHydra>(Owner);
	}
	...
	void TickActive(float DeltaTime)
	{
		...
		if (!bSentStartEvent && FadeInTimer >= MaxTime - KINDA_SMALL_NUMBER)
		{
			bSentStartEvent = true;
			USanctuaryBossSplineRunHydraEventHandler::Trigger_Start_GlowThroat(Owner);
		}
	}
};