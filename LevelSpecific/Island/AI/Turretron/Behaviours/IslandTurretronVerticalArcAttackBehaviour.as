class UIslandTurretronVerticalArcAttackBehaviour : UBasicBehaviour
{
	...
	private void TargetDirectly(float DeltaTime)
	{
		...
		FVector PlayerDir = (TargetPlayerLoc - Owner.ActorCenterLocation).GetSafeNormal2D();
		FVector Dir;
		float Angle = 20.0;
		Dir = CurrentAimCenterDir.RotateTowards(FVector::DownVector, Angle * 0.1);
		//Dir = Dir.RotateTowards(PlayerDir, NumFiredProjectiles * Angle * 2.0 / float(CurrentNumProjectilesInBurst));
		TowardsDir = FVector::UpVector;
		FVector TurretRightVector = Turret.Mesh_Turret.GetRightVector(); // Owner.ActorRightVector is fix, not rotating.
		float PlayerSideDot = TurretRightVector.DotProduct(PlayerDir);
		PrintScaled("PlayerSideDot=" + Math::Abs(PlayerSideDot) + ", Cos(5deg)=" + Math::Cos(Math::DegreesToRadians(85)), Duration=0.0);
		if (Math::Abs(PlayerSideDot) > Math::Cos(Math::DegreesToRadians(85)))
		{
			FVector PlayerSideDir = TurretRightVector.DotProduct(PlayerDir) > 0 ? TurretRightVector : TurretRightVector * -1.0;
			Debug::DrawDebugLine(Owner.ActorCenterLocation, Owner.ActorCenterLocation + TurretRightVector*500, FLinearColor::Green, Duration=3.0);
			Debug::DrawDebugLine(Owner.ActorCenterLocation, Owner.ActorCenterLocation + PlayerSideDir*500, FLinearColor::Red, Duration=3.0);
			//TowardsDir = FVector::UpVector.RotateTowards(PlayerSideDir, 45);
			const float RotationSpeed = 10;
			float Delta = RotationSpeed * DeltaTime;
			TowardsDir = TowardsDir.SlerpTowards(PlayerSideDir, Delta);
			Debug::DrawDebugLine(Owner.ActorCenterLocation, Owner.ActorCenterLocation + TowardsDir*500, FLinearColor::Gray, Duration=3.0);
		}

		Dir = Dir.RotateTowards(TowardsDir, NumFiredProjectiles * Angle * 2.0 / float(CurrentNumProjectilesInBurst));
		...
		Debug::DrawDebugLine(Owner.ActorCenterLocation, Owner.ActorCenterLocation + CurrentDir*500, FLinearColor::Blue, Duration=3.0);
