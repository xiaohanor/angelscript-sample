class UPinballMagnetAttractionMovementResolver : USweepingMovementResolver
{
	default RequiredDataType = UPinballMagnetAttractionMovementData;

	private const UPinballMagnetAttractionMovementData MovementData;
	...
	void PrepareResolver(const UBaseMovementData Movement) override
	{
		...
		MovementData = Cast<UPinballMagnetAttractionMovementData>(Movement);
	...
	{
		if(MagnetDrone::IsImpactMagnetic(Contact, true))
		{
	...
	void ResolveAndApplyMovementRequest(UHazeMovementComponent MovementComponent) override
	{
		...
		{
			auto AttractionComp = UPinballProxyMagnetAttractionComponent::Get(MovementComponent.Owner);
