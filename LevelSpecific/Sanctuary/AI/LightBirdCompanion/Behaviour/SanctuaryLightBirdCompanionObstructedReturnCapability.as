class USanctuaryLightBirdCompanionObstructedReturnCapability : UHazeCapability
{
	...
	void PreTick(float DeltaTime)
	{
		...
		else
		{
			...
			if (ObstructionDuration > Settings.ObstructedDetectDuration)
				CompanionComp.State = ELightBirdCompanionState::Obstructed; // Set this here to deactivate follow etc
		}
	}
	...
	bool IsObstructed(float LOSInterval)
	{
		...
		if (MoveComp.HasAnyValidBlockingContacts())
			return true;
		if (!Owner.ActorLocation.IsWithinDist(CompanionComp.Player.ActorLocation, Settings.AutoRecallRange * 1.1))
			return true;
	...
	void OnActivated()
	{
		CompanionComp.State = ELightBirdCompanionState::Obstructed; // Makes sure remote side gets this		
	...
	void ComposeMovement(float DeltaTime)
	{
		...
		if (RemainingDistance > Settings.AutoRecallRange * 1.1)
			Speed.AccelerateTo(Settings.FollowFarSpeed * 4.0, 1.0, DeltaTime);
		else if (RemainingDistance > Settings.ObstructedNearRange)
