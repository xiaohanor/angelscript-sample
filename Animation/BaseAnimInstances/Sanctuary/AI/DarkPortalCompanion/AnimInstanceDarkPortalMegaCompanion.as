UCLASS(Abstract)
class UAnimInstanceDarkPortalMegaCompanion : UAnimInstanceAIBase
{
	...
	bool bPhysDisablePortalActive = false;
	...
	void BlueprintUpdateAnimation(float DeltaTime)
	{
		...
		if (LastLocation.Size() > KINDA_SMALL_NUMBER)
		{
			...
			FakeVelocity = DeltaMove / Math::Max(KINDA_SMALL_NUMBER, DeltaTime);
		...
		if (PhysComp == nullptr)
			PhysComp = UHazePhysicalAnimationComponent::Get(HazeOwningActor);
		...
		if (!bPhysDisablePortalActive && bPortalActive)
		{
			bPhysDisablePortalActive = true;
			PhysComp.Disable(this);
		}
		else if (!bPhysDisableTeleport && !bPhysDisablePortalActive)
		{
			PhysComp.ClearDisable(this);
		}

		if (LocomotionState == EDarkPortalCompanionLocomotionState::LaunchStart || LocomotionState == EDarkPortalCompanionLocomotionState::PortalExit)
		{
			PhysComp.Disable(this);
			TwistAlpha = 0;
		}
		...
		if (LocomotionState == EDarkPortalCompanionLocomotionState::PortalExit)
		{
			TrailAlpha = 0;
		}
		else
		...
		DashPlayRate *= MegaPlayRate;

		LocomotionPlayRate = (FVector(SpeedForward, SpeedRight, SpeedUp).Size() / 300) * 1.0 + 0.3;
		LocomotionPlayRate = Math::Clamp(LocomotionPlayRate, 0.0, 3.0);
		LocomotionPlayRate *= MegaPlayRate;
		...
		if (Math::Abs(RotationRate.X) > SMALL_NUMBER && DeltaTime > SMALL_NUMBER)
		{
		...
		/*
		Print("TrailAlpha: " + TrailAlpha, 0.f);
