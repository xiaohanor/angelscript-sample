class UPlayerJumpCapability : UHazePlayerCapability
{
	...
	float GraceTime = 0.12;
	float JumpCooldownTime = 0.09;
	...
	void PreTick(float DeltaTime)
	{
		...
		{
			// Impulses always cancel the jump grace period afterward
			JumpComp.bIsInJumpGracePeriod = false;
			JumpComp.JumpGraceTimer = MAX_flt;
		}
		else if (MoveComp.IsInAir())
		{
			JumpComp.JumpGraceTimer += DeltaTime;
			JumpComp.bIsInJumpGracePeriod = JumpComp.JumpGraceTimer <= GraceTime;
			
			JumpComp.JumpCooldown = 0.0;
		}
		else if ((MoveComp.IsOnWalkableGround() || MoveComp.HasCustomMovementStatus(n"Perching")) && Time::GameTimeSeconds > JumpComp.PreventJumpGraceUntilGameTime)
		{
			JumpComp.JumpGraceTimer = 0.0;
			JumpComp.bIsInJumpGracePeriod = true;

			if(JumpComp.JumpCooldown < JumpCooldownTime)
			{
				JumpComp.JumpCooldown += DeltaTime;
				JumpComp.bIsJumpOnCooldown = JumpComp.JumpCooldown <= JumpCooldownTime;
			}
		}
	}
	...
	void OnActivated()
	{
		...
		FVector VerticalVelocity = MoveComp.WorldUp * JumpComp.Settings.Impulse;
		FVector HorizontalVelocity;

		if(Time::GetGameTimeSince(FloorMotionComp.Data.SprintTurnaroundActivatedAtTime) <= SprintComp.Settings.TurnAroundSlowdownDuration)
		{
			//We recently initiated a sprint turnaround
			HorizontalVelocity = Player.ActorForwardVector
		...
		{
			// Add jump impulse
			HorizontalVelocity = MoveComp.HorizontalVelocity.GetSafeNormal()
				* Math::Max(AirMotionComp.Settings.HorizontalMoveSpeed * MoveComp.MovementInput.Size(),
							MoveComp.HorizontalVelocity.Size());
		}
		...
		UPlayerCoreMovementEffectHandler::Trigger_GroundJump_Started(Player);
	}
	...
	void OnDeactivated()
	{
		...
		UPlayerCoreMovementEffectHandler::Trigger_GroundJump_CancelledOrReachedApex(Player);
	}
