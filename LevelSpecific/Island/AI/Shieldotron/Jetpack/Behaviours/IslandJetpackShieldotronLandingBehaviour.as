class UIslandJetpackShieldotronLandingBehaviour : UBasicBehaviour
{
	...
	default Speed.AddDefaultKey(0.0, 0.2);
	default Speed.AddDefaultKey(0.5, 0.6);
	...
	bool ShouldActivate() const
	{
		...
		//	return false;
		if (JetpackComp.CurrentFlyState != EIslandJetpackShieldotronFlyState::IsLanding)
			return false;
	...
	{	
		...
		// Select a TargetLocation on ground, if no ground is found, set a cooldown
		FHazeTraceSettings Trace = Trace::InitChannel(ECollisionChannel::WorldGeometry);
		Trace.UseLine();
		FHitResult HitResult = Trace.QueryTraceSingle(Owner.ActorLocation, Owner.ActorLocation - Owner.ActorUpVector * 2000);
	...
	{
		Super::OnDeactivated();
		JetpackComp.SetCurrentFlyState(EIslandJetpackShieldotronFlyState::IsGrounded);
	}
