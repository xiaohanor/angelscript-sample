...
class UMeltdownBossPhaseTwoEditorSubsystem : UHazeEditorSubsystem
{
	bool bHasBossPhaseTwoManager = false;
	...
	{
		auto Manager = AMeltdownBossPhaseTwoManager::Get();
		bHasBossPhaseTwoManager = (Manager != nullptr);
	}

	void ShowMeshesFromOtherWorlds(AMeltdownBossPhaseTwoLevelAnchor ClosestLevelAnchor, TArray<AMeltdownBossPhaseTwoLevelAnchor> LevelAnrchors)
	{
		...
		for (AMeltdownBossPhaseTwoLevelAnchor OffsetActor : LevelAnrchors)
		{
			if (OffsetActor == ClosestLevelAnchor)
				continue;
			...
			for (AActor Actor : OffsetActor.Level.Actors)
			{
				...
				for (auto MeshComp : StaticMeshes)
				{
					...
					MeshTransform.Location = MeshTransform.Location - OffsetActor.ActorLocation + ClosestLevelAnchor.ActorLocation;
	...
	{
		if (!bHasBossPhaseTwoManager || Editor::IsPlaying())
			return;
		...
		TArray<AMeltdownBossPhaseTwoLevelAnchor> LevelAnchors = TListedActors<AMeltdownBossPhaseTwoLevelAnchor>().GetArray();
		LevelAnchors.Sort();

		AMeltdownBossPhaseTwoLevelAnchor ClosestLevelAnchor;
		float ClosestDist = MAX_flt;
		for (auto OffsetActor : LevelAnchors)
		{
			float Dist = OffsetActor.ActorLocation.Distance(Editor::GetEditorViewLocation());
			if (Dist < ClosestDist)
			{
				ClosestLevelAnchor = OffsetActor;
				ClosestDist = Dist;
			}
		}
		...
		for (auto OffsetActor : LevelAnchors)
		{
			auto WorldButton = ButtonBox
				...
				.BorderBox()
					.BackgroundColor(
							OffsetActor == ClosestLevelAnchor
								? FLinearColor::Red
								: FLinearColor::Black
						)
					.SlotPadding(1)
			...
			if (WorldButton.WasClicked())
			{
				Editor::SetEditorViewLocation(
					Editor::GetEditorViewLocation()
						- ClosestLevelAnchor.ActorLocation
		...
		if (bShowOtherWorlds)
		{
			ShowMeshesFromOtherWorlds(ClosestLevelAnchor, LevelAnchors);
