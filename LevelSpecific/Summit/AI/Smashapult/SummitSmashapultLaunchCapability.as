asset SummitSmashapultLaunchGravity of UMovementGravitySettings
{
	GravityAmount = 5000.0;
}

class USummitSmashapultLaunchCapability : UHazeCapability
{
	...
	AAISummitSmashapult Smashapult;
	...
	USummitSmashapultComponent SmashapultComp;
	...
	const float HorizontalLaunchSpeed = 3000.0;

	UFUNCTION(BlueprintOverride)
	void Setup()
	{
		Smashapult = Cast<AAISummitSmashapult>(Owner);
		...
		SmashapultComp = USummitSmashapultComponent::GetOrCreate(Owner);
	...
	bool ShouldActivate() const
	{
		...
		if(!SmashapultComp.LaunchTarget.IsSet())
			return false;
	...
	void OnActivated()
	{
		Smashapult.ApplySettings(SummitSmashapultLaunchGravity, this);
		MoveComp.AccRotation.SnapTo(Owner.ActorRotation);
		PrevLocation = Owner.ActorLocation;
		AccUp.SnapTo(Owner.ActorUpVector.Rotation());

		FVector LaunchVelocity = Trajectory::CalculateVelocityForPathWithHorizontalSpeed(Owner.ActorLocation, SmashapultComp.LaunchTarget.Value, MoveComp.GravityForce, HorizontalLaunchSpeed);
		Smashapult.SetActorVelocity(LaunchVelocity);

		Smashapult.BlockCapabilities(BasicAITags::Behaviour, this);
	}

	UFUNCTION(BlueprintOverride)
	void OnDeactivated()
	{
		Smashapult.UnblockCapabilities(BasicAITags::Behaviour, this);

		Smashapult.ClearSettingsByInstigator(this);
	}
	...
	{	
		...
		// // Just stay in place, turning towards target
		// Movement.AddVelocity(FVector::ZeroVector);
