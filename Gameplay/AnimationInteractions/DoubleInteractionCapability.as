class UDoubleInteractionCapability : UInteractionCapability
{
	ADoubleInteractionActor InteractionActor;
	...
	{
		return CheckInteraction.Owner.IsA(ADoubleInteractionActor);
	}
	...
	void OnActivated(FInteractionCapabilityParams Params)
	{
		...
		InteractionActor = Cast<ADoubleInteractionActor>(ActiveInteraction.Owner);
		...
		if (ActiveInteraction.MovementSettings.HasMovement())
		{
			...
			Player.RootComponent.AttachToComponent(ActiveInteraction, NAME_None,
	...
	void TickActive(float DeltaTime)
	{
		...
		if (Player.HasControl() && InteractionActor.CompletionLock.IsAcquired(Player))
		{
			if (InteractionState.Status == EDoubleInteractionStatus::None)
			{
				...
				if (InteractionState.PlayerState[0].bIsInteracting
					&& InteractionState.PlayerState[1].bIsInteracting
					&& !InteractionState.PlayerState[Player].bHasCanceled)
				{
...
struct FDoubleInteractionCapabilityParams
{
	ADoubleInteractionActor InteractionActor;
	UInteractionComponent Component;
};

class UDoubleInteractionEnterAnimationCapability : UHazePlayerCapability
{
	...
	bool ShouldActivate(FDoubleInteractionCapabilityParams& Params) const
	{
		...
		{
			ADoubleInteractionActor Interaction = Cast<ADoubleInteractionActor>(PlayerInteractionsComp.ActiveInteraction.Owner);
	...
	bool ShouldDeactivate() const
	{
		...
		if (InteractionActor.State.PlayerState[Player].bHasCompletedInteraction
			|| InteractionActor.State.Status == EDoubleInteractionStatus::LockedIn)
		{
...
class UDoubleInteractionMHAnimationCapability : UHazePlayerCapability
{
	default CapabilityTags.Add(n"BlockedByCutscene");
	default CapabilityTags.Add(n"BlockedWhileDead");
	...
	bool ShouldActivate(FDoubleInteractionCapabilityParams& Params) const
	{
		if (IsValid(PlayerInteractionsComp.ActiveInteraction))
		{
			...
			if (IsValid(Interaction)
				&& Interaction.State.PlayerState[Player].bHasEnterCompleted
				&& !Interaction.State.PlayerState[Player].bHasMHStarted
			)
			{
				Params.InteractionActor = Interaction;
				Params.Component = PlayerInteractionsComp.ActiveInteraction;
...
class UDoubleInteractionCancelAnimationCapability : UHazePlayerCapability
{
	default CapabilityTags.Add(n"BlockedByCutscene");
	default CapabilityTags.Add(n"BlockedWhileDead");
	...
	bool ShouldActivate(FDoubleInteractionCapabilityParams& Params) const
	{
		if (IsValid(PlayerInteractionsComp.ActiveInteraction))
		{
			...
			if (IsValid(Interaction)
				&& Interaction.State.PlayerState[Player].bHasMHStarted
				&& Interaction.State.PlayerState[Player].bHasCanceled
			)
			{
				Params.InteractionActor = Interaction;
				Params.Component = PlayerInteractionsComp.ActiveInteraction;
...
class UDoubleInteractionCompletedAnimationCapability : UHazePlayerCapability
{
	default CapabilityTags.Add(n"BlockedByCutscene");
	default CapabilityTags.Add(n"BlockedWhileDead");
	...
	bool ShouldActivate(FDoubleInteractionCapabilityParams& Params) const
	{
		if (IsValid(PlayerInteractionsComp.ActiveInteraction))
		{
			...
			if (IsValid(Interaction)
				&& Interaction.State.PlayerState[Player].bHasMHStarted
				&& Interaction.State.PlayerState[Player].bHasCompletedInteraction
			)
			{
				Params.InteractionActor = Interaction;
				Params.Component = PlayerInteractionsComp.ActiveInteraction;
