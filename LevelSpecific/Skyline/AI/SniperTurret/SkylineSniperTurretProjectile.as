struct FSkylineSniperTurretProjectileImpactParams
{
	UPROPERTY()
	FHitResult Hit;
}

class USkylineSniperTurretProjectileEventHandler : UHazeEffectEventHandler
{
	UFUNCTION(BlueprintEvent)
	void OnImpact(FSkylineSniperTurretProjectileImpactParams Params) {};
}

class ASkylineSniperTurretProjectile : AHazeActor
{
	...
	UPROPERTY(DefaultComponent)
	USkylineSniperTurretProjectileComponent ProjectileComp;
	...
	void Tick(float DeltaTime)
	{
		...
		{
			USkylineSniperTurretSettings Settings = USkylineSniperTurretSettings::GetSettings(ProjectileComp.Launcher);
		...
		if (Hit.bBlockingHit)
		{
			...
			FSkylineSniperTurretProjectileImpactParams ImpactParams;
			ImpactParams.Hit = Hit;

			USkylineSniperTurretProjectileEventHandler::Trigger_OnImpact(ProjectileComp.GetLauncher(), ImpactParams);
		}
