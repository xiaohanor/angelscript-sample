class UIslandRedBlueAssaultShootBulletCapability : UHazePlayerCapability
{
	...
	UIslandRedBlueAssaultSettings AssaultSettings;
	...
	void Setup()
	{
		...
		AssaultSettings = UIslandRedBlueAssaultSettings::GetSettings(Player);
	}
	...
	void ShootBullet(UIslandRedBlueTargetableComponent Targetable, float TimeSinceShouldHaveShot)
	{
		...
		BulletParams.BulletInitialSpeed = AssaultSettings.BulletInitialSpeed;
		BulletParams.BulletSpeedAcceleration = AssaultSettings.BulletSpeedAcceleration;
		BulletParams.BulletSpeedMax = AssaultSettings.BulletSpeedMax;
		BulletParams.BulletDamageMultiplier = AssaultSettings.BulletDamageMultiplier;
	...
	float GetCurrentCooldownBetweenBullets(float TimeSinceShouldHaveShot) const
	{
		...
		const float CurveAlpha = Math::Clamp(TimeSinceStartShooting / AssaultSettings.CoolDownBetweenBulletsCurveDuration, 0.0, 1.0);
		const float CurveValue = AssaultSettings.CoolDownBetweenBulletsCurve.GetFloatValue(CurveAlpha);
		return Math::Lerp(AssaultSettings.StartCoolDownBetweenBullets, AssaultSettings.TargetCoolDownBetweenBullets, CurveValue);
	}
}