...
class USandSharkPathfollowingCapability : UHazeCapability
{
	...
	void TickActive(float DeltaTime)
	{
		...
		if (NeedsNewPath())
		{
			EBasicPathfindingResult Result = FindPath(OwnLoc + Owner.ActorForwardVector, Destination, MoveToComp.PathIndex);
	...
	bool NeedsNewPath()
	{
		...
		if (bAccuratePath)
		{
			...
			// If we have been moving away from path, update path.
			if (MoveToComp.Owner.ActorLocation.DistSquared(MoveToComp.Path.Points[0]) > Math::Square(Settings.UpdatePathDistance))
				return true;
