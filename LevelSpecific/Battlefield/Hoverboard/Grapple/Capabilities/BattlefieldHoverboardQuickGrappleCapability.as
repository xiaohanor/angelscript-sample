...
class UBattlefieldHoverboardQuickGrappleCapability : UHazePlayerCapability
{
	...
	void TickActive(float DeltaTime)
	{
		if(MoveComp.PrepareMove(Movement))
		{
			if(HasControl())
			{
				...
				float CurvedAlpha = GrappleSettings.SpeedCurve.GetFloatValue(Alpha);

				FVector NewLoc = Math::Lerp(StartLocation, TargetLocation, CurvedAlpha);
				FVector HeightOffset = MoveComp.WorldUp * GrappleSettings.HeightCurve.GetFloatValue(Alpha) * (GrappleComp.GrappleHeightOffset / 2);
				NewLoc += HeightOffset;
			...
			MoveComp.ApplyMoveAndRequestLocomotion(Movement, n"HoverboardGrappling");
