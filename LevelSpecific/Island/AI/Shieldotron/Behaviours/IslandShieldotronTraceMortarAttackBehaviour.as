class UIslandShieldotronTraceMortarAttackBehaviour : UBasicBehaviour
{
	...
	FName MortarAttackStartToken = n"MortarAttackStart";
	...
	float ReleaseMortarAttackStartTokenTimer = 0.0;
	...
	bool CanAttack() const
	{
		...
		if (!TargetComp.HasValidTarget())
			return false;
		if (!UGentlemanComponent::GetOrCreate(TargetComp.Target).CanClaimToken(MortarAttackStartToken, this))
			return false;
		...
		FVector LaunchVelocity = FVector::UpVector * Settings.MortarAttackProjectileEndSpeed; // Approximates the launch direction
		if (!IslandShieldotron::HasClearMortarTrajectory(MortarLauncherLeftComp.WorldLocation, AttackLocation, LaunchVelocity, Settings.MortarAttackLandingSteepness))
			return false;
	...
	void OnActivated()
	{
		...
		GentComp = UGentlemanComponent::Get(TargetComp.Target);
		GentComp.ClaimToken(MortarAttackStartToken, this);
		ReleaseMortarAttackStartTokenTimer = Math::RandRange(0.2, 0.4);
	...
	void OnDeactivated()
	{
		...
		GentComp.ReleaseToken(MortarAttackStartToken, this);
	...
	{		
		ReleaseMortarAttackStartTokenTimer -= DeltaTime;
		if (ReleaseMortarAttackStartTokenTimer < 0.0)
		{
			GentComp.ReleaseToken(MortarAttackStartToken, this);
		}
