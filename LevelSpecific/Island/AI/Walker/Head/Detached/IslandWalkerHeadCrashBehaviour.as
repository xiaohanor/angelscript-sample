class UIslandWalkerHeadCrashBehaviour : UBasicBehaviour
{
	...
	bool ShouldActivate() const
	{
		...
		if (Stump == nullptr)
			return false;
		if (Stump.Health > Settings.HeadCrashHealthThreshold)
			return false;
		return true;
	}
	...
	void OnActivated()
	{
		...
		{
			float DistSqr = Site.Area.WorldLocation.DistSquared2D(IdealCrashLoc);
		...
		// Never fall below arena height
		HeadComp.MovementFloor.Apply(Arena.Height, this);
		...
		Stump.Health = Settings.HeadCrashHealthThreshold;
	...
	void TickActive(float DeltaTime)
	{
		...
		{
			// Crashed, but not yet recovering
	...
	{
		if (!HeadComp.ShockWaves.IsValidIndex(iShockwave))
			return;

		FVector Epicenter = Arena.GetAtArenaHeight(Owner.ActorLocation + Owner.ActorForwardVector * 200.0);
