class AWorldSpinCartActor : AHazeActor
{
	...
	UPROPERTY(DefaultComponent, Attach = Mesh)
	UStaticMeshComponent Wheel03;

	UPROPERTY(DefaultComponent, Attach = Mesh)
	UStaticMeshComponent Wheel04;
	...
	bool bHasHitEndConstraint;
	bool bHasHitStartConstraint;
	...
	void BeginPlay()
	{
		...
		SplineFollow.OnStartHit.AddUFunction(this, n"HitStartConstraint");
		SplineFollow.OnEndHit.AddUFunction(this, n"OnEndHit");
	}

	UFUNCTION()
	private void OnEndHit(float Strength)
	{
		bHasHitEndConstraint = true;
	}

	UFUNCTION()
	private void HitStartConstraint(float Strength)
	{
		bHasHitStartConstraint = true;
	}
	...
	void Tick(float DeltaSeconds)
	{
		...
		{	
			...
			if(Spindirection < -0.1 && !bHasHitEndConstraint)
			{
				bHasHitStartConstraint = false;

				UMeltdownWorldSpinCartEventHandler::Trigger_CartMoving(this, FMeltdownWorldSpinCartMovingEvent(Mesh));

				for(UStaticMeshComponent Wheel : Wheels)
					Wheel.AddLocalRotation(FRotator(-90,0,0) * DeltaSeconds);
			}
			else
				UMeltdownWorldSpinCartEventHandler::Trigger_CartNotMoving(this, FMeltdownWorldSpinCartMovingEvent(Mesh));

			if(Spindirection > 0.1 && !bHasHitStartConstraint)
			{
				bHasHitEndConstraint = false;
			 ...
			 if (bHasHitEndConstraint)
		 		for(UStaticMeshComponent Wheel : Wheels)
		 			Wheel.AddLocalRotation(FRotator(0,0,0) * DeltaSeconds);

			if (bHasHitStartConstraint)
		 		for(UStaticMeshComponent Wheel : Wheels)
		 			Wheel.AddLocalRotation(FRotator(0,0,0) * DeltaSeconds);
