class UPlayerFallingLowMantleClimbUpCapability : UHazePlayerCapability
{
	...
	bool ShouldActivate() const
	{
		...
		if (MantleComp.GetState() != EPlayerLedgeMantleState::FallingLowEnter)
			return false;
	...
	void OnActivated()
	{
		...
		MantleComp.SetState(EPlayerLedgeMantleState::FallingLowExit);
	...
	void OnDeactivated()
	{
		...
		float ExitSpeed = Math::Lerp(200, 350, Math::Clamp(MoveComp.MovementInput.DotProduct(Player.ActorForwardVector), 0, 1));
		Player.SetActorVelocity(Player.ActorForwardVector * ExitSpeed);

		MoveComp.UnFollowComponentMovement(this);
		MantleComp.StateCompleted(EPlayerLedgeMantleState::FallingLowExit);
	...
	void TickActive(float DeltaTime)
	{
		if (MoveComp.PrepareMove(Movement))
		{
			if(HasControl())
			{
				...
				MoveAlpha = Math::Clamp(ActiveTimer / MantleComp.Settings.FallingLowMantleClimbDuration, 0, 1);
