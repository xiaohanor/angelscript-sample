class USummitKnightSwoopTackleBehaviour : UBasicBehaviour
{
	...
	USummitKnightSwoopTackleBehaviour(bool bObstacles)
	{
	...
	void OnActivated()
	{
		...
		if (TargetComp.HasValidTarget())
			MainTarget = Cast<AHazePlayerCharacter>(TargetComp.Target);
		...
		SwoopTargetSpeed = Owner.ActorLocation.Dist2D(Destination) / Math::Max(0.2, TraversalDuration);
		...
		USummitKnightEventHandler::Trigger_OnSwoopAggroTelegraph(Owner, FSummitKnightPlayerParams(MainTarget));
	}
	...
	void TickActive(float DeltaTime)
	{
		...
		else if (!bTraversing)
		{
			...
			USummitKnightEventHandler::Trigger_OnSwoopAggroStart(Owner, FSummitKnightPlayerParams(MainTarget));
		}
		...
		if (bTraversing)
		{
			...
			if (!Destination.IsWithinDist2D(Owner.ActorLocation, 200.0))
				DestinationComp.MoveTowardsIgnorePathfinding(Destination, SwoopSpeed.Value);
		}
		else if (bRecovering)
		{
			// Stop
		...
		if (!bRecovering)
		{
			if (!Destination.IsWithinDist2D(Owner.ActorLocation, 500.0))
	...
	{
		// Try to run over target, or pass to other side of arena if there is no target
		FVector TargetLoc = (KnightComp.Arena.Center + (Owner.ActorLocation + Owner.ActorForwardVector * 1000.0)) * 0.5;
		if (TargetComp.HasValidTarget())
			TargetLoc = TargetComp.Target.ActorLocation;
		else if (TargetComp.IsValidTarget(Game::Zoe))
			TargetLoc = Game::Zoe.ActorLocation;
		else if (TargetComp.IsValidTarget(Game::Mio))
			TargetLoc = Game::Mio.ActorLocation;
		
		FVector Dest = Owner.ActorLocation + ((TargetLoc - Owner.ActorLocation).GetSafeNormal2D() * KnightComp.Arena.Radius * 2.0);	
		Dest = KnightComp.Arena.GetClampedToArena(Dest, 600.0);
		return Dest;
	}
	...
	void CrumbHitPlayer(AHazePlayerCharacter Player)
	{
		...
		USummitKnightEventHandler::Trigger_OnSwoopAggroHitPlayer(Owner, FSummitKnightPlayerParams(Player));		
	}
	...
	void CrumbEndSwoop(FAreaDenialZoneObstacleSpawnParameters ObstacleParameters)
	{
		...
		USummitKnightEventHandler::Trigger_OnSwoopAggroEnd(Owner);
	}
