UCLASS(Abstract)
class AWingsuitBossBlockadeMine : AHazeActor
{
	...
	private AWingsuitBossMineBlockadeTargetActor CurrentTarget;
	...
	void SpawnProjectile(AWingsuitBossMineBlockadeTargetActor In_CurrentTarget, UHazeActorNetworkedSpawnPoolComponent In_SpawnPool, UWingsuitBossMineLauncher In_Launcher, UWingsuitBossMineLauncher In_ShootComponent, UWingsuitBossSettings In_Settings)
	{
		...
		FVector TargetLocation = CurrentTarget.ActorLocation;
	...
	{
		if(CurrentMoveState == EWingsuitBossMineMoveState::Shoot)
		{
		...
		else if(CurrentMoveState == EWingsuitBossMineMoveState::Floating)
		{
			...
			if (Time::GetGameTimeSince(TimeOfLand) >= 5)
			{
				Kill();
				return;
			}
