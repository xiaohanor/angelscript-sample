...
class UPlayerLedgeGrabCapability : UHazePlayerCapability
{
	...
	void OnActivated(FPlayerLedgeGrabData LedgeGrabActivationData)
	{
		...
		// if(LedgeGrabComp.GetState() != EPlayerLedgeGrabState::Dash)
		// {
		...
		LedgeGrabComp.SetState(EPlayerLedgeGrabState::LedgeGrab);
		...
		//Quick fix for spring arm compression when climbing over ledges as SnowMonkey
		if(LedgeGrabComp.bSnowMonkeyLedgeGrabActivated)
		{
	...
	void TickActive(float DeltaTime)
	{
		if (MoveComp.PrepareMove(Movement))
		{
			if (HasControl())
			{
				...
				//Update our ledge grab data and make sure we still have a valid ledge. - [AL](This was previously done in the shimmy trace above)
				FPlayerLedgeGrabData LedgeGrabData;
				LedgeGrabComp.TraceForLedgeGrabAtLocation(Player, -LedgeGrabComp.Data.WallImpactNormal.ConstrainToPlane(MoveComp.WorldUp), Player.ActorLocation, LedgeGrabData, this, IsDebugActive());
				LedgeGrabComp.Data = LedgeGrabData;
