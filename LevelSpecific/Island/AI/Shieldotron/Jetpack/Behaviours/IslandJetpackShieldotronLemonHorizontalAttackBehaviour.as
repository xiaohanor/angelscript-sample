class UIslandJetpackShieldotronLemonHorizontalAttackBehaviour : UBasicBehaviour
{
	...
	const EIslandJetpackShieldotronAttack LemonHorizontalAttack = EIslandJetpackShieldotronAttack::LemonHorizontal;

	bool bIsSweepingRight = false;
	...
	bool ShouldActivate() const
	{
		if (Super::ShouldActivate() == false)
			return false;
		if (!AttackComp.CanAttack(LemonHorizontalAttack))
			return false;
		if (!WantsToAttack())
			return false;
	...
	void OnActivated()
	{
		...
		bIsSweepingRight = !bIsSweepingRight; // toggle direction
	...
	void OnDeactivated()
	{
		...
		AttackComp.NextAttackAfter(LemonHorizontalAttack);
	}
	...
	FVector CalculateAimDir()
	{
		FVector AimDir;
		float Angle = 30.0;
		FVector StartDir = bIsSweepingRight ? Owner.ActorRightVector * -1.0 : Owner.ActorRightVector;
		AimDir = CurrentAimCenterDir.RotateTowards(StartDir, Angle);
		FVector RotateTowardsDir = bIsSweepingRight ? Owner.ActorRightVector : Owner.ActorRightVector * -1.0;
		AimDir = AimDir.RotateTowards(RotateTowardsDir, NumFiredProjectiles * Angle * 2.0 / (float(JetpackSettings.LemonAttackBurstNumber)));
		return AimDir;
	}
	...
	void UpdateAnimationAimSpace(FVector AimDir)
	{
		...
		AimComp.DesiredYaw = YawSign * AimYawAngle;
	}
