event void FIslandHoverCraftSignature();

class AIslandHoverCraft : AHazeActor
{
	...
	UPROPERTY()
	FIslandHoverCraftSignature OnReachedDestination;

	UPROPERTY()
	FIslandHoverCraftSignature OnRespawnComplete;
	...
	void Tick(float DeltaSeconds)
	{
		...
		if (!bIsDestroyed)
			BobbingRoot.SetRelativeLocation(FVector::UpVector * Math::Sin((Time::PredictedGlobalCrumbTrailTime * BobSpeed + BobOffset)) * BobHeight);
		...
		if (bIsDestroyed && bSmoothDestruction == false)
		{
			...
			if(HasControl())
			{
				FRotator BobbingRotation = Math::RInterpConstantTo(BobbingRoot.RelativeRotation, FRotator(Math::Sin((Time::PredictedGlobalCrumbTrailTime * 10 + BobOffset)), Math::Sin((Time::PredictedGlobalCrumbTrailTime * 10 + BobOffset)), 0), DeltaSeconds, 15);
	...
	{
		UIslandHoverCraftEffectHandler::Trigger_OnStopMovingOnSpline(this, FIslandHoverCraftEffectParams(this));
	}
	...
	void ActivateSplineMove()
	{
		...
		UIslandHoverCraftEffectHandler::Trigger_OnStartMovingOnSpline(this, FIslandHoverCraftEffectParams(this));
	}
	...
	void Activate()
	{
		...
		UIslandHoverCraftEffectHandler::Trigger_OnStarted(this, FIslandHoverCraftEffectParams(this));
	}
	...
	{
		MoveAnimation.Stop();
		UIslandHoverCraftEffectHandler::Trigger_OnStopped(this, FIslandHoverCraftEffectParams(this));
	}
	...
	void ExplodeActor()
	{
		UIslandHoverCraftEffectHandler::Trigger_OnDestroyed(this, FIslandHoverCraftEffectParams(this));
		BP_ExplodeActor();
		bIsExploded = true;
	}
	...
	void HandleRedOvercharge()
	{
		...
		UIslandHoverCraftEffectHandler::Trigger_OnOverCharged(this, FIslandHoverCrafOnOverChargeEffectParams(Game::Mio));
	}
	...
	void HandleBlueOvercharge()
	{
		...
		UIslandHoverCraftEffectHandler::Trigger_OnOverCharged(this, FIslandHoverCrafOnOverChargeEffectParams(Game::Zoe));
	}
...
UCLASS(Abstract)
class UIslandHoverCraftEffectHandler : UHazeEffectEventHandler
{
	// Will trigger when the hovercraft starts moving on the spline
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnStartMovingOnSpline(FIslandHoverCraftEffectParams Params) {}

	// Will trigger when the hovercraft stops moving on the spline
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnStopMovingOnSpline(FIslandHoverCraftEffectParams Params) {}

	// Will trigger when the hovercraft starts falling
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnStarted(FIslandHoverCraftEffectParams Params) {}

	// Will trigger when the hovercraft stops falling
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnStopped(FIslandHoverCraftEffectParams Params) {}

	// Will trigger when the hovercraft starts exploding
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnDestroyed(FIslandHoverCraftEffectParams Params) {}

	// Will trigger when a player has caused a overcharge.
	UFUNCTION(BlueprintEvent, Meta = (AutoCreateBPNode))
	void OnOverCharged(FIslandHoverCrafOnOverChargeEffectParams Params) {}
}

struct FIslandHoverCraftEffectParams
{
	FIslandHoverCraftEffectParams(AIslandHoverCraft In_HoverCraft)
	{
		HoverCraft = In_HoverCraft;
	}

	AIslandHoverCraft HoverCraft;
}

struct FIslandHoverCrafOnOverChargeEffectParams
{
	FIslandHoverCrafOnOverChargeEffectParams(AHazePlayerCharacter InPlayer)
	{
		Player = InPlayer;
	}
